
import { rollup } from "npm:rollup";
import { minify } from "npm:terser";

let bundle = await rollup({
  input: "mod.js",
});

let { output } = await bundle.generate({
  format: "umd",
  name: "tinycolor",
});
let minified = await minify(output[0].code);
const preamble = `// This file is autogenerated.
// Ideally it wouldn't exist, but it's here for backwards compatibility for people
// using it directly from a CDN. Please use the npm package which exports both CJS and ESM.
`;

Deno.writeTextFileSync("tinycolor.js", `// This file is autogenerated.
// Ideally it wouldn't exist, but it's here for backwards compatibility for people
// using it directly from a CDN. Please use the npm package which exports both CJS and ESM.
` + output[0].code);

Deno.writeTextFileSync("./cjs/tinycolor.js", `// This file is autogenerated.
// It's here for backwards compatibility with CJS.
` + output[0].code);

Deno.writeTextFileSync("dist/tinycolor-min.js", preamble + minified.code);

const test_template = Deno.readTextFileSync("./cjs/test_cjs_template.js");
const test_content = Deno.readTextFileSync("test.js");
Deno.writeTextFileSync(
  "./cjs/test_cjs.js",
  test_template.replace("// CONTENT_GOES_HERE", test_content.substring(test_content.indexOf("// TEST_BEGINS_HERE"), test_content.length - 1)),
);
