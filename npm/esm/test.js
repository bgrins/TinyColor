...
import { assertEquals, assert, } from "./deps/deno.land/std@0.168.0/testing/asserts.js";
...
dntShim.Deno.test("With Ratio", function () {
    ...
    assertEquals(tinycolor.fromRatio({ r: 1, g: 0, b: 0, a: 0.5 }).toRgbString(), "rgba(255, 0, 0, 0.5)", "alpha works when ratio is parsed");
...
dntShim.Deno.test("Without Ratio", function () {
    ...
    assertEquals(tinycolor({ r: 0.1, g: 0.1, b: 0.1 }).toHexString(), "#000000", "000000");
...
dntShim.Deno.test("RGB Text Parsing", function () {
    ...
    assert(tinycolor.equals({ r: 200, g: 100, b: 0 }, "rgb 200 100 0"));
    assert(tinycolor.equals({ r: 200, g: 100, b: 0 }, "rgb 200 100 0"));
    assert(tinycolor.equals({ r: 200, g: 100, b: 0, a: 0.4 }, "rgba 200 100 0 .4"));
...
dntShim.Deno.test("Percentage RGB Text Parsing", function () {
    ...
    assert(tinycolor.equals({ r: "90%", g: "45%", b: "0%" }, "rgb 90% 45% 0%"));
    assert(tinycolor.equals({ r: "90%", g: "45%", b: "0%" }, "rgb 90% 45% 0%"));
    assert(tinycolor.equals({ r: "90%", g: "45%", b: "0%", a: 0.4 }, "rgba 90% 45% 0% .4"));
...
dntShim.Deno.test("HSL parsing", function () {
    assertEquals(tinycolor({ h: 251, s: 100, l: 0.38 }).toHexString(), "#2400c2", "to hex");
    assertEquals(tinycolor({ h: 251, s: 100, l: 0.38 }).toRgbString(), "rgb(36, 0, 194)", "to rgb");
    assertEquals(tinycolor({ h: 251, s: 100, l: 0.38 }).toHslString(), "hsl(251, 100%, 38%)", "to hsl");
...
dntShim.Deno.test("Invalid alpha should normalize to 1", function () {
    ...
    assertEquals(tinycolor({ r: 255, g: 20, b: 10, a: 0.5 }).toRgbString(), "rgba(255, 20, 10, 0.5)", ".5");
...
dntShim.Deno.test("toString() with alpha set", function () {
    var redNamed = tinycolor.fromRatio({ r: 255, g: 0, b: 0, a: 0.6 }, { format: "name" });
    var transparentNamed = tinycolor.fromRatio({ r: 255, g: 0, b: 0, a: 0 }, { format: "name" });
    var redHex = tinycolor.fromRatio({ r: 255, g: 0, b: 0, a: 0.4 }, { format: "hex" });
...
dntShim.Deno.test("isReadable", function () {
    ...
    assert(!tinycolor.isReadable("#ff0088", "#8822aa", {
        level: "AAA",
        size: "small",
    ...
    assert(!tinycolor.isReadable("#ff0088", "#5c1a72", {
        level: "AAA",
        size: "small",
    }), "not readable - AAA small");
    assert(!tinycolor.isReadable("#ff0088", "#5c1a72", {
    ...
    assert(!tinycolor.isReadable("#ff0088", "#2e0c3a", {
        level: "AAA",
        size: "small",
    }), "not readable - AAA small");
